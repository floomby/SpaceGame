  ...
  if (impulseSlot !== null) {
    swarm.transitions.push({
      trigger: (_, npc, ___, target) => {
        if (Math.random() < 0.02 && !!target && l2Norm(npc.player.position, target.position) < 2500) {
  ...
  constructor(ship: string, team: Faction, id: number) {
    ...
    switch (Math.floor(Math.random() * 4)) {
      ...
      case 1:
        ...
        this.currentState = makeStateGraph(estimateEffectivePrimaryRange(def), true, 3000, 3, mineSlot, impulseSlot);
        break;
      case 2:
      case 3:
        this.player = equip(this.player, 1, "Tomahawk Missile", true);
        this.currentState = makeStateGraph(estimateEffectivePrimaryRange(def), true, 2500, 3, mineSlot, impulseSlot);
        break;
    }
